...
public class RegionSelection_Cam extends View {
    ...
    private RegionSelection_Cam.OnUpCallback mCallback = null;

    public interface OnUpCallback {
        void onRectFinished(android.graphics.Rect rect);
    }

    public RegionSelection_Cam(final Context context) {
        super(context);
        init();
    }

    public RegionSelection_Cam(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RegionSelection_Cam(final Context context, final AttributeSet attrs, final int defStyle) {
     ...
     */
    public void setOnUpCallback(RegionSelection_Cam.OnUpCallback callback) {
        mCallback = callback;
    }
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mCallback != null) {
                    mCallback.onRectFinished(new android.graphics.Rect(Math.min(mStartX, mEndX), Math.min(mStartY, mEndY),
